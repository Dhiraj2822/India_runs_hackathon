...
## File to build: `src/scoring_engine.py`
## Depends on: `src/models.py`, `src/nlp_engine.py`, `config.py`
## Do NOT start this module until ALL 9 Module 2 tests pass.
...
Two scoring stages:
- **Stage 1** (`compute_stage1_score`): Fast structured scoring on all 100,000 candidates.
  No embeddings. Runs in seconds. Reduces 100K → top 5,000.
- **Stage 2** (`compute_base_score`): Full scoring on the Stage 1 top 5,000.
  Uses embeddings + all signals. Produces `CandidateScore` with 6 components.

No edge case logic here. No penalties or bonuses.
Those live in Module 4 and operate on the output of this module.
...
from src.models import CandidateProfile, JobDescription, CandidateScore
...
## FUNCTION: `compute_stage1_score(candidate: CandidateProfile, jd: JobDescription) -> float`
...
## PRIVATE STAGE 1 FUNCTIONS

### `_s1_title_relevance(candidate: CandidateProfile) -> float` *(private)*

```
Purpose: Is this person's current title relevant to an AI/ML engineering role?
...
### `_s1_skill_count(candidate: CandidateProfile, jd: JobDescription) -> float` *(private)*

```
Purpose: How many of the JD required skills does the candidate have?
...
### `_s1_availability(candidate: CandidateProfile) -> float` *(private)*

```
Purpose: Is this candidate actually available and active?
...
elif days_inactive <= 60:  recency = 0.75
elif days_inactive <= 90:  recency = 0.60
elif days_inactive <= 180: recency = 0.40
else:                      recency = 0.15   ← >6 months = nearly unavailable

availability = (open_weight * 0.60) + (recency * 0.40)
return max(0.0, min(1.0, availability))
```

### `_s1_domain_fit(candidate: CandidateProfile) -> float` *(private)*

```
Purpose: Is the candidate in a tech/AI-adjacent domain at all?
...
for career_item in candidate.career_history[:3]:   ← only recent 3 roles
  for ti in tech_industries:
...
## FUNCTION: `compute_base_score(candidate: CandidateProfile, jd: JobDescription) -> CandidateScore`
...
## PRIVATE STAGE 2 FUNCTIONS

### `_s2_semantic(candidate: CandidateProfile, jd: JobDescription) -> float` *(private)*

```
Purpose: Semantic similarity between full candidate profile and the JD.
Uses pre-computed normalised embeddings from Module 2.

sim = cosine_similarity(candidate.embedding, jd.embedding)
return max(0.0, min(1.0, float(sim)))
```

### `_s2_skill_quality(candidate: CandidateProfile, jd: JobDescription) -> float` *(private)*

```
Purpose: Quality-weighted skill score. Goes beyond presence to measure
...
for req in JD_REQUIRED_SKILLS:
  skill = _find_skill(req, candidate_skills_map)  ← handles synonyms
  if skill is None:
...
### `_find_skill(skill_name: str, skills_map: dict) -> Optional[SkillEntry]` *(private)*
...
### `_s2_career_fit(candidate: CandidateProfile, jd: JobDescription) -> float` *(private)*

```
Purpose: How well does the candidate's career history match the JD?
Checks: title trajectory, production ML experience, company type.

score = 0.0

# Component 1: Title trajectory (30% of career_fit)
# Did the candidate ever hold an AI/ML relevant title?
ai_title_count = 0
...
# Look for evidence of shipped systems, not just research
production_signals = [
...
# Consulting firms = bad signal per JD
...
### `_s2_experience(candidate: CandidateProfile, jd: JobDescription) -> float` *(private)*

```
Purpose: Years of experience relative to JD requirements (5-9 years).
...
### `_s2_behavioral(candidate: CandidateProfile) -> float` *(private)*

```
Purpose: Engagement and availability signals from the Redrob platform.
...
# Response rate: 30% weight — will they actually reply to Redrob's recruiters?
response_score = sig.recruiter_response_rate   ← already 0.0-1.0

# Profile completeness: 15% — more complete = more serious
completeness = sig.profile_completeness_score / 100.0

# GitHub activity: 25% — coding activity signal
...
# Interview completion: 15% — reliability signal
interview = sig.interview_completion_rate

# Market demand: 15% — are other recruiters interested?
# saved_by_recruiters_30d and search_appearance_30d as proxy
...
### `_s2_education(candidate: CandidateProfile) -> float` *(private)*

```
Purpose: Education tier and field relevance.
...
## MODULE 3 TESTS — `tests/test_module3.py`

ALL 10 tests must pass before proceeding to Module 4.
...
1. `pytest tests/test_module3.py -v` shows ALL 10 tests passing
2. Stage 1 on 100,000 candidates completes in under 10 seconds
3. Stage 2 on 5,000 candidates (with pre-computed embeddings) completes in under 15 seconds

Only then: proceed to MODULE_4_EDGE_CASES.md.

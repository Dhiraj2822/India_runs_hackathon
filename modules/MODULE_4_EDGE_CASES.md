# MODULE 4 — EDGE CASES & HONEYPOT DETECTION
## File to build: `src/edge_cases.py`
## Depends on: `src/models.py`, `config.py`
## Do NOT start until ALL 10 Module 3 tests pass.
...
1. **Honeypot detection**: Identifies the ~80 candidates with impossible profiles.
   Sets `candidate.is_honeypot = True` and `candidate.honeypot_reasons` list.
   Honeypots get `final_score = 0.0` — they must NOT appear in the top 100.

2. **Edge case adjustments**: Applies penalties and bonuses to `CandidateScore.base_score`
   to produce `CandidateScore.final_score`. These reflect real-world recruiting signals
   that the scoring engine's 6 components do not fully capture.

### CRITICAL: Additive-only rule
Every edge case is a private function named `_ec_*`.
The orchestrator `apply_edge_cases()` calls them in a fixed order.
When a new edge case is needed in future: append a new `_ec_*` function
and add ONE call at the end of the orchestrator, before the final_score line.
Never modify existing `_ec_*` functions. Never reorder existing calls.
...
## MASTER FUNCTION: `apply_edge_cases(candidate, score, jd) -> CandidateScore`
...
### `_detect_honeypot(candidate: CandidateProfile) -> CandidateProfile` *(private)*
...
### `_ec_consulting_only(candidate, score, jd) -> CandidateScore` *(private)*
...
### `_ec_not_open_to_work(candidate, score, jd) -> CandidateScore` *(private)*
...
### `_ec_inactive_long(candidate, score, jd) -> CandidateScore` *(private)*
...
### `_ec_job_hopper(candidate, score, jd) -> CandidateScore` *(private)*
...
### `_ec_keyword_stuffer(candidate, score, jd) -> CandidateScore` *(private)*
...
### `_ec_long_notice_period(candidate, score, jd) -> CandidateScore` *(private)*
...
### `_ec_primary_wrong_domain(candidate, score, jd) -> CandidateScore` *(private)*
...
### `_ec_preferred_location(candidate, score, jd) -> CandidateScore` *(private)*
...
### `_ec_github_active(candidate, score, jd) -> CandidateScore` *(private)*
...
### `_ec_assessment_excellence(candidate, score, jd) -> CandidateScore` *(private)*
...
### `_ec_internal_promotion(candidate, score, jd) -> CandidateScore` *(private)*
...
### `_ec_open_source_signal(candidate, score, jd) -> CandidateScore` *(private)*
...
### `_ec_short_notice_bonus(candidate, score, jd) -> CandidateScore` *(private)*
...
### `_ec_willing_to_relocate(candidate, score, jd) -> CandidateScore` *(private)*
...
## MODULE 4 TESTS — `tests/test_module4.py`

ALL 10 tests must pass before proceeding to Module 5.
...
1. `pytest tests/test_module4.py -v` shows ALL 10 tests passing
2. No honeypot passes through with `final_score > 0`
3. Marketing Manager with AI skills gets penalised below 0.60
4. runtime for edge cases on 200 candidates: under 2 seconds

Only then: proceed to MODULE_5_RANKER.md.

# BACKEND_SCHEMA.md
## All Data Models — Based on Actual candidate_schema.json + Dataset Analysis
## These go in `src/models.py`. Build this file BEFORE any module.

---

## src/models.py (COMPLETE FILE CONTENT)
...
The `submission.csv` file has exactly these 4 columns in this order:

```
candidate_id  | string  | CAND_XXXXXXX format (7 digits)
rank          | integer | 1-100, each value exactly once, ascending
score         | float   | 0.0-1.0, non-increasing (rank 1 = highest score)
reasoning     | string  | 1-2 sentences, specific to this candidate
```

---

## Field Ranges Reference (for validation)

| Field | Range | Notes |
|-------|-------|-------|
| years_of_experience | 0.0 – 50.0 | float |
| proficiency | "beginner"\|"intermediate"\|"advanced"\|"expert" | enum |
| tier | "tier_1"\|"tier_2"\|"tier_3"\|"tier_4"\|"unknown" | enum |
| recruiter_response_rate | 0.0 – 1.0 | float |
| github_activity_score | -1 or 0.0-100.0 | -1 = no GitHub |
| offer_acceptance_rate | -1 or 0.0-1.0 | -1 = no history |
| notice_period_days | 0 – 180 | integer |
| company_size | "1-10"\|"11-50"\|"51-200"\|"201-500"\|"501-1000"\|"1001-5000"\|"5001-10000"\|"10001+" | enum |
| preferred_work_mode | "remote"\|"hybrid"\|"onsite"\|"flexible" | enum |
